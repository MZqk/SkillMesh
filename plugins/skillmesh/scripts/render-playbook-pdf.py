...
def render_step(step, styles):
    ...
    if commands:
        flowables.extend([
            ...
            paragraph("SkillMesh 不会执行以下命令。复制前请检查项目环境和影响范围。", styles["warning"]),
...
def render_document(payload):
    ...
    story = [
        Spacer(1, 34 * mm),
        rich_paragraph("SKILLMESH · DEVELOPMENT PLAYBOOK", styles["cover_eyebrow"]),
        ...
        Spacer(1, 10 * mm),
        paragraph("本手册只提供步骤、提示词、命令建议和验收门；SkillMesh 不会自动运行命令或修改项目。", styles["callout"]),
        PageBreak(),
    ...
    def page_frame(canvas, doc):
        ...
        canvas.setFont(FONT, 7.2)
        canvas.setFillColor(INK_MUTED)
        canvas.drawString(22 * mm, 9 * mm, f"SkillMesh · {status}")
