...
const API_ERROR_MESSAGE = {
  ...
  "ecosystem-skill-review-required": "请先读取并审阅这份 Skill 原文",
  "ecosystem-reviewed-content-changed": "上游 Skill 原文已变化，请重新读取后再记录",
  ...
  "skill-not-found": "这份本机 Skill 已不存在，请重新扫描",
  "skill-content-changed-refresh-required": "Skill 原文已变化，请重新扫描后再审阅",
  ...
  "quick-skill-state-conflict": "快速 Skill 偏好已在另一端更新，请刷新后重试",
  "quick-skill-workflow-not-found": "快速 Skill 工作流已不存在，请重新选择",
  "quick-skill-stage-not-found": "快速 Skill 阶段已变化，请重新选择",
};
...
const state = {
  ...
  quickDeck: {
    preferences: loadQuickDeckPreferences(localStorage),
    sections: null,
    snapshot: null,
    selectedItem: null,
    sending: false,
...
const elements = {
  ...
  quickDeckButton: document.querySelector("#quick-deck-button"),
  quickDeckButtonMeta: document.querySelector("#quick-deck-button-meta"),
  quickDeckDialog: document.querySelector("#quick-deck-dialog"),
  quickStageStrip: document.querySelector("#quick-stage-strip"),
  quickDeckLayout: document.querySelector("#quick-deck-layout"),
  quickDeckBrowser: document.querySelector("#quick-deck-browser"),
  quickDeckContent: document.querySelector("#quick-deck-content"),
  quickDeckCount: document.querySelector("#quick-deck-count"),
  quickDeckPlaybook: document.querySelector("#quick-deck-playbook"),
  quickDeckCatalog: document.querySelector("#quick-deck-catalog"),
  quickUsePanel: document.querySelector("#quick-use-panel"),
  quickUseForm: document.querySelector("#quick-use-form"),
  quickUseBack: document.querySelector("#quick-use-back"),
  quickUseSource: document.querySelector("#quick-use-source"),
  quickUseName: document.querySelector("#quick-use-name"),
  quickUseDescription: document.querySelector("#quick-use-description"),
  quickUseTask: document.querySelector("#quick-use-task"),
  quickUseAgent: document.querySelector("#quick-use-agent"),
  quickUseAgentNote: document.querySelector("#quick-use-agent-note"),
  quickUseOutput: document.querySelector("#quick-use-output"),
  quickUsePreview: document.querySelector("#quick-use-preview"),
  quickUseSendNote: document.querySelector("#quick-use-send-note"),
  quickUseSend: document.querySelector("#quick-use-send"),
...
function applyWorkflow(workflow, { syncPlaybook = true } = {}) {
  ...
  renderWorkflowState();
  if (syncPlaybook) playbookUi?.setWorkflow(workflow);
}
...
function renderInventory() {
  ...
  rebuildQuickDeckSections();
  if (elements.quickDeckDialog.open) renderQuickDeckCards();
...
function renderWorkflowPicker() {
  ...
  const status = workflow.status === "confirmed" ? `执行基线 v${workflow.confirmedVersion}` : "可生成方案";
...
function renderWorkflowState() {
  ...
  elements.workflowBadge.textContent = confirmed ? `执行基线 v${workflow.confirmedVersion}` : "共享草案";
  ...
  elements.workflowHistory.textContent = history.length
    ? `保留 ${history.length} 个执行基线版本 · 最近更新 ${formatDate(workflow.updatedAt)}`
    : `可直接生成方案 · 最近更新 ${formatDate(workflow.updatedAt)}`;
...
function renderWorkflowConfirmationControls() {
  ...
  if (confirmation.confirmed) {
    elements.workflowConfirmationReadiness.textContent = "当前工作流已纳入执行基线";
  ...
  elements.workflowConfirmationReadiness.textContent = "在执行方案中统一锁定，无需单独确认";
  elements.editWorkflowDefinition.textContent = "编辑确认信息";
  elements.confirmWorkflow.textContent = "统一锁定";
  elements.confirmWorkflow.title = "请在执行方案中锁定基线";
  return;
}
...
async function openWorkflow(id, { assess = true, announce = false } = {}) {
  ...
  await syncQuickSkillContext().catch((error) => console.warn("quick Skill context sync failed", error));
...
function selectStage(stageId, { reveal = false, trigger = null } = {}) {
  ...
  renderStages();
  renderInspector();
  syncQuickSkillContext()
    .then(() => elements.quickDeckDialog.open ? loadQuickDeckContext() : null)
    .catch((error) => console.warn("quick Skill stage sync failed", error));
  if (reveal) {
...
function renderStages() {
  ...
  for (const stage of state.plan.stages) {
    ...
    result.append(
      ...
      node("small", { text: `${stage.coverage.confirmed || 0}/${stage.coverage.total} 已确认 · ${stage.coverage.matched}/${stage.coverage.total} 文本证据 · ${stage.candidates.length} 候选` }),
    );
    card.append(result);
...
function renderInspector() {
  ...
  scoreSummary.append(
    ...
    node("span", { text: `文本证据 ${Math.round(stage.coverage.ratio * 100)}%` }),
    node("span", { text: `人工确认 ${Math.round((stage.coverage.confirmedRatio || 0) * 100)}%` }),
  ...
  for (const capability of stage.capabilityCoverage) {
    ...
    for (const external of capability.externalCandidates || []) {
      ...
      const reviewBlocked = !external.reviewedContentHash
        || ["high", "critical"].includes(external.reviewedSeverity);
      externalCard.append(
      ...
      externalCard.append(node("span", {
        text: external.reviewedContentHash
          ? `已审阅 sha256:${external.reviewedContentHash.slice(0, 16)}… · 静态线索 ${external.reviewedSeverity || "none"}`
          : "未绑定已审阅原文指纹 · 不可安装",
      }));
      if (external.chain) {
        externalCard.append(node("span", {
      ...
      if (external.status === "suggested") {
        const accept = node("button", {
          text: reviewBlocked ? "原文审阅未通过" : "接受为安装候选",
          attributes: { type: "button", ...(reviewBlocked ? { disabled: "", title: "需要已审阅指纹，且静态线索不能为高风险或严重。" } : {}) },
        });
        ...
        const add = node("button", {
          text: reviewBlocked ? "审阅门未通过" : "加入安装计划",
          attributes: { type: "button", ...(reviewBlocked ? { disabled: "" } : {}) },
        });
...
function syncHeaderActions() {
  const movable = [elements.workspaceButton, elements.rescan, elements.exportJson];
...
function canonicalEntries() {
  ...
  return [...byContent.entries()].map(([key, copies]) => {
    copies.sort((left, right) => skillPreference(right) - skillPreference(left) || left.path.localeCompare(right.path));
    ...
    return {
      ...
      skill: {
        ...representative,
        providers: unique(copies.map((copy) => copy.provider)),
        supportedAgents: unique(copies.flatMap((copy) => copy.supportedAgents || [])),
        allowedTools: unique(copies.flatMap((copy) => copy.allowedTools || [])),
        triggers: unique(copies.flatMap((copy) => copy.triggers || [])),
        keywords: unique(copies.flatMap((copy) => copy.keywords || [])),
        invocation: copies.map((copy) => copy.invocation).find(Boolean) || "",
        enabled: copies.some((copy) => copy.enabled !== false),
      },
    };
  });
}

function rebuildQuickDeckSections() {
  state.quickDeck.sections ||= {
    context: null,
  ...
  const favoriteCount = state.quickDeck.preferences.favorites.length;
  const recentCount = state.quickDeck.preferences.recent.length;
  elements.quickDeckButtonMeta.textContent = favoriteCount || recentCount
    ? `当前阶段 · 收藏 ${favoriteCount} · 最近 ${recentCount}`
    : "当前阶段 · 收藏 · 最近";
  return state.quickDeck.sections;
}

function applyQuickSkillSnapshot(snapshot) {
  state.quickDeck.snapshot = snapshot;
  state.quickDeck.preferences = snapshot.state || state.quickDeck.preferences;
  state.quickDeck.sections = snapshot.sections;
  const workflowId = snapshot.state?.activeWorkflowId;
  if (workflowId) state.workspace.activeWorkflowId = workflowId;
  rebuildQuickDeckSections();
  return snapshot;
}

async function fetchQuickSkillState() {
  const preferences = await (await api("/api/quick-skill-state")).json();
  state.quickDeck.preferences = preferences;
  rebuildQuickDeckSections();
  return preferences;
}

async function updateQuickSkillState(operation, { retryOnConflict = true } = {}) {
  try {
    const preferences = await (await api("/api/quick-skill-state", {
      method: "PATCH",
      body: JSON.stringify({
        expectedRevision: state.quickDeck.preferences.revision || 0,
        operation,
      }),
    })).json();
    state.quickDeck.preferences = preferences;
    ...
    if (error.status !== 409 || !retryOnConflict) throw error;
    await fetchQuickSkillState();
    return updateQuickSkillState(operation, { retryOnConflict: false });
  }
}

async function syncQuickSkillContext() {
  const workflowId = state.activeWorkflow?.id || null;
  const stageId = workflowId ? state.selectedStageId || null : null;
  const currentWorkflowId = state.quickDeck.preferences.activeWorkflowId || null;
  const currentStageId = workflowId
    ? state.quickDeck.preferences.activeStageByWorkflow?.[workflowId] || null
    : null;
  if (workflowId === currentWorkflowId && (!workflowId || stageId === currentStageId)) return;
  await updateQuickSkillState({ type: "select-context", workflowId, stageId });
}

async function migrateLegacyQuickSkillPreferences() {
  const preferences = loadQuickDeckPreferences(localStorage);
  const migration = await (await api("/api/quick-skill-state/migrate", {
    method: "POST",
    body: JSON.stringify({
      preferences,
      activeWorkflowId: state.workspace.activeWorkflowId || null,
      selectedStageId: state.selectedStageId || null,
    }),
  })).json();
  state.quickDeck.preferences = migration.state;
  if (migration.state?.legacyWebMigrationCompleted) {
    try { localStorage.removeItem(QUICK_DECK_STORAGE_KEY); } catch { /* Server state remains authoritative. */ }
  }
  const workflowId = migration.state?.activeWorkflowId;
  if (workflowId) {
    state.workspace.activeWorkflowId = workflowId;
    state.selectedStageId = migration.state.activeStageByWorkflow?.[workflowId] || state.selectedStageId;
  }
  rebuildQuickDeckSections();
  return migration;
}

function quickDeckSourceLabel(item) {
  if (item.source === "current") return item.stepTitle ? `当前步骤 · ${item.stepTitle}` : "当前阶段相关";
  if (item.source === "favorite") return "已收藏";
  return item.usedAt ? `最近使用 · ${formatDate(item.usedAt)}` : "最近使用";
}

function quickDeckSection(title, description, section, emptyText) {
  const wrapper = node("section", { className: "quick-deck-section" });
  const heading = node("div", { className: "quick-deck-section-heading" }, [
    node("div", {}, [node("h3", { text: title }), node("p", { text: description })]),
    node("span", { text: section.hidden ? `${section.items.length} / ${section.total}` : String(section.total) }),
  ]);
  wrapper.append(heading);
  if (!section.items.length) {
    wrapper.append(node("div", { className: "quick-deck-empty", text: emptyText }));
    return wrapper;
  }
  const grid = node("div", { className: "quick-skill-grid" });
  const favorites = new Set(state.quickDeck.preferences.favorites);
  for (const item of section.items) {
    const favorite = favorites.has(item.contentHash);
    const card = node("article", {
      className: `quick-skill-card${state.quickDeck.selectedItem?.contentHash === item.contentHash ? " selected" : ""}`,
      dataset: { source: item.source },
    });
    const top = node("div", { className: "quick-skill-card-top" }, [
      node("span", { text: quickDeckSourceLabel(item) }),
    ]);
    ...
    favoriteButton.addEventListener("click", handle(async () => {
      await updateQuickSkillState({
        type: "set-favorite",
        contentHash: item.contentHash,
        favorite: !favorite,
      });
      await loadQuickDeckContext();
      toast(favorite ? "已取消收藏" : "已加入收藏");
    }));
    top.append(favoriteButton);
    const context = item.source === "current" && item.rationale
      ? node("p", { className: "quick-skill-rationale", text: item.rationale })
      : null;
    const agentDeclarations = item.supportedAgents || [];
    const agents = agentDeclarations.includes("*")
      ? "任意 Agent"
      : agentDeclarations.length
        ? `${agentDeclarations.slice(0, 2).join(" / ")}${agentDeclarations.length > 2 ? ` +${agentDeclarations.length - 2}` : ""}`
        : "未限定 Agent";
    const providers = item.providers.length > 2
      ? `${item.providers.slice(0, 2).join(" / ")} +${item.providers.length - 2}`
      : item.providers.join(" / ") || "本机 Skill";
    const action = node("button", {
      className: "button button-primary quick-skill-use",
      text: "使用",
      attributes: { type: "button", "aria-label": `使用 ${item.name}` },
    });
    action.addEventListener("click", () => selectQuickSkill(item));
    card.append(
      top,
      node("h4", { text: item.name }),
      node("p", { className: "quick-skill-description", text: item.description }),
    );
    if (context) card.append(context);
    card.append(node("footer", {}, [
      node("span", { text: `${providers} · ${agents}` }),
      action,
    ]));
    grid.append(card);
  }
  wrapper.append(grid);
  if (section.hidden) wrapper.append(node("p", {
    className: "quick-deck-hidden-note",
    text: `另有 ${section.hidden} 项已收起；这里不继续展开卡片墙。`,
  }));
  return wrapper;
}

function renderQuickStageStrip() {
  const context = state.quickDeck.sections?.context;
  elements.quickStageStrip.replaceChildren();
  if (!context) {
    elements.quickStageStrip.append(node("div", {}, [
      node("span", { text: "当前阶段" }),
      node("strong", { text: "尚无可用的执行阶段" }),
      node("small", { text: "仍可从收藏或最近使用中选择 Skill。" }),
    ]));
    return;
  }
  elements.quickStageStrip.append(node("div", {}, [
    node("span", { text: context.source === "playbook" ? "执行方案 · 当前阶段" : "技能地图 · 当前阶段" }),
    node("strong", { text: context.stageTitle }),
    node("small", { text: context.summary || "只展示与此阶段直接相关的本机 Skill。" }),
  ]));
}

function renderQuickDeckCards() {
  const sections = state.quickDeck.sections || rebuildQuickDeckSections();
  renderQuickStageStrip();
  elements.quickDeckContent.replaceChildren(
    quickDeckSection(
      "当前阶段相关",
      "优先依据执行方案中的当前未完成步骤；没有执行方案时使用当前技能地图阶段。",
      sections.current,
      "当前阶段暂时没有达到证据门槛的本机 Skill，可查看执行方案或完整标本册。",
    ),
    quickDeckSection(
      "收藏",
      "固定保留你主动标记的高频 Skill。",
      sections.favorites,
      "尚无收藏。在任意卡片右上角点击 ☆ 即可加入。",
    ),
    quickDeckSection(
      "最近使用",
      "只保留最近 12 次记录，并排除上方已经出现的 Skill。",
      sections.recent,
      "完成一次交接后，Skill 会自动出现在这里。",
    ),
  );
  elements.quickDeckCount.textContent = sections.totalHidden
    ? `当前展示 ${sections.totalVisible} 项，另有 ${sections.totalHidden} 项收起；不会展开完整 Skill 清单。`
    : `当前展示 ${sections.totalVisible} 项；不会展开完整 Skill 清单。`;
}

function currentAgentSender() {
  const skillmeshSender = window.skillmeshMcp?.sendFollowUpMessage;
  if (typeof skillmeshSender === "function") return skillmeshSender.bind(window.skillmeshMcp);
  const openAiSender = window.openai?.sendFollowUpMessage;
  if (typeof openAiSender === "function") return openAiSender.bind(window.openai);
  return null;
}

function quickUseContext(item) {
  return {
    workflowTitle: state.activeWorkflow?.goal || state.activeWorkflow?.reference?.name || "",
    stageTitle: item.stageTitle || state.quickDeck.sections?.context?.stageTitle || "",
    stepTitle: item.stepTitle || "",
    acceptanceCriteria: item.acceptanceCriteria || [],
    invocationPrompt: item.invocationPrompt || item.invocation || "",
  };
}

function selectedQuickTargetLabel() {
  const option = elements.quickUseAgent.selectedOptions[0];
  return option?.dataset.agentLabel || option?.textContent || "当前 Agent";
}

function quickUseOutputs() {
  return [...new Set(elements.quickUseOutput.value
    .split(/\r?\n/)
    .map((item) => item.trim())
    .filter(Boolean))];
}

function quickUsePrompt() {
  const item = state.quickDeck.selectedItem;
  if (!item) throw new Error("请先选择一份 Skill");
  return buildSkillHandoff({
    skill: item,
    task: elements.quickUseTask.value,
    targetAgent: selectedQuickTargetLabel(),
    expectedOutputs: quickUseOutputs(),
    context: quickUseContext(item),
  });
}

function updateQuickUsePreview() {
  if (!state.quickDeck.selectedItem) return;
  if (!elements.quickUseTask.value.trim() || !quickUseOutputs().length) {
    elements.quickUsePreview.textContent = "填写任务和至少一项预期产物后，这里会生成完整交接内容。";
    return;
  }
  elements.quickUsePreview.textContent = quickUsePrompt();
}

function updateQuickSendMode() {
  const direct = Boolean(currentAgentSender());
  elements.quickUseSend.textContent = direct ? "发送给当前 Agent" : "复制给当前 Agent";
  elements.quickUseSendNote.textContent = direct
    ? "点击后直接追加到当前 Agent 对话；不会在网页中执行 Skill。"
    : "当前浏览器没有 Agent 会话桥；点击后复制完整任务，粘贴到目标 Agent 即可使用。";
}

function selectQuickSkill(item) {
  state.quickDeck.selectedItem = item;
  elements.quickDeckLayout.dataset.view = "compose";
  elements.quickUsePanel.hidden = false;
  elements.quickUseSource.textContent = quickDeckSourceLabel(item);
  elements.quickUseName.textContent = item.name;
  elements.quickUseDescription.textContent = item.description;
  elements.quickUseTask.value = item.taskSuggestion || "";
  elements.quickUseOutput.value = (item.expectedOutputs || []).join("\n");

  const targets = buildTargetAgentOptions({
    skill: item,
    targets: state.installation.status?.targets || [],
    preferredTargetAgents: state.activeWorkflow?.requirements?.targetAgents || [],
  });
  elements.quickUseAgent.replaceChildren(...targets.map((target) => {
    const suffix = target.current
      ? ""
      : target.preferred
        ? " · 工作流目标"
        : target.detected
          ...
          : "";
    return node("option", {
      text: `${target.label}${suffix}`,
      dataset: { agentLabel: target.label },
      attributes: { value: target.value },
    });
  }));
  elements.quickUseAgent.value = "current";
  const compatibleLabels = targets.slice(1).map((target) => target.label);
  elements.quickUseAgentNote.textContent = compatibleLabels.length
    ? `兼容 Agent：${compatibleLabels.slice(0, 4).join("、")}${compatibleLabels.length > 4 ? ` 等 ${compatibleLabels.length} 个` : ""}；默认使用当前 Agent。`
    : "该 Skill 未声明其他兼容 Agent；默认使用当前 Agent。";
  updateQuickSendMode();
  updateQuickUsePreview();
  renderQuickDeckCards();
  elements.quickUseTask.focus();
}

function closeQuickUsePanel() {
  state.quickDeck.selectedItem = null;
  elements.quickDeckLayout.dataset.view = "cards";
  elements.quickUsePanel.hidden = true;
  renderQuickDeckCards();
  elements.quickDeckContent.querySelector(".quick-skill-use")?.focus();
}

async function loadQuickDeckContext() {
  const statusPromise = state.installation.status
    ? Promise.resolve(state.installation.status)
    : fetchInstallationStatus().catch((error) => {
      console.warn("quick deck target lookup failed", error);
      return null;
    });
  const params = new URLSearchParams();
  if (state.activeWorkflow?.id) params.set("workflowId", state.activeWorkflow.id);
  if (state.selectedStageId) params.set("stageId", state.selectedStageId);
  const snapshotPromise = api(`/api/quick-skill-deck${params.size ? `?${params}` : ""}`)
    .then((response) => response.json());
  await statusPromise;
  applyQuickSkillSnapshot(await snapshotPromise);
  renderQuickDeckCards();
}

async function openQuickDeck() {
  state.quickDeck.selectedItem = null;
  elements.quickDeckLayout.dataset.view = "cards";
  elements.quickUsePanel.hidden = true;
  elements.quickDeckContent.replaceChildren(node("div", {
    className: "quick-deck-loading",
    text: "正在整理当前阶段的 Skill…",
  }));
  elements.quickStageStrip.replaceChildren(node("div", {}, [
    node("span", { text: "当前阶段" }),
    node("strong", { text: "正在读取执行上下文…" }),
  ]));
  if (!elements.quickDeckDialog.open) elements.quickDeckDialog.showModal();
  await loadQuickDeckContext();
}

async function copyTextAwaited(value) {
  try {
    await navigator.clipboard.writeText(value);
    return;
  } catch {
    const fallback = node("textarea", { text: value, attributes: { readonly: "" } });
    ...
    fallback.remove();
    if (!copied) throw new Error("无法访问剪贴板，请展开预览后手动复制");
  }
}

async function submitQuickUse() {
  if (state.quickDeck.sending) return;
  const item = state.quickDeck.selectedItem;
  if (!item) throw new Error("请先选择一份 Skill");
  const prompt = quickUsePrompt();
  const sender = currentAgentSender();
  state.quickDeck.sending = true;
  elements.quickUseSend.disabled = true;
  elements.quickUseSend.textContent = sender ? "正在发送…" : "正在复制…";
  try {
    if (sender) {
      try {
        await sender({ prompt });
        toast(`已将“${item.name}”任务发送给当前 Agent`);
      } catch (error) {
        console.warn("current Agent bridge failed; copied instead", error);
        await copyTextAwaited(prompt);
        toast("宿主发送失败，完整任务已复制");
      }
    } else {
      await copyTextAwaited(prompt);
      toast(`“${item.name}”任务已复制，可直接粘贴给当前 Agent`);
    }
    try {
      await updateQuickSkillState({ type: "record-use", contentHash: item.contentHash });
      await loadQuickDeckContext();
    } catch (error) {
      console.warn("quick deck recent-use sync failed after handoff", error);
      toast("任务已交接，但最近使用同步失败；请勿重复发送");
    }
  } finally {
    state.quickDeck.sending = false;
    elements.quickUseSend.disabled = false;
    updateQuickSendMode();
  }
}
...
function renderLocalCatalogList(entries) {
  ...
  for (const entry of visible) {
    ...
    item.append(
      ...
      node("small", { text: `${(entry.skill.providers || [entry.skill.provider]).join(" / ")} · ${entry.skill.scope}` }),
...
function localSkillDocumentKey(entry) {
  return entry?.skill?.contentHash || entry?.key || "";
}

async function loadLocalSkillDocument(entry, { refresh = false } = {}) {
  const review = state.catalog.localDocument;
  const key = localSkillDocumentKey(entry);
  if (!key) return;
  ...
  renderLocalCatalogDetail(entry);
  try {
    const payload = await (await api(`/api/skills/${encodeURIComponent(key)}/content?maxChars=${128 * 1024}`)).json();
  ...
  const current = filteredCatalogEntries().find((item) => item.key === state.catalog.selectedKey);
  if (state.catalog.mode === "local" && localSkillDocumentKey(current) === key) {
    renderLocalCatalogDetail(current);
  }
}

function renderLocalUsageGuide(skill) {
  const section = node("section", { className: "specimen-section local-usage-guide" });
  section.append(node("h4", { text: "何时使用与如何调用" }));
  const grid = node("div", { className: "local-usage-grid" });
  const when = node("article");
  when.append(node("h5", { text: "何时使用" }));
  if (skill.triggers?.length) {
    when.append(list(skill.triggers, ""));
  } else {
    when.append(node("p", {
      text: `未显式声明 triggers。可先按 description 判断：${skill.description || "作用说明也缺失，请先阅读原文。"}`,
    }));
  }

  const agents = (skill.supportedAgents || []).filter((agent) => agent !== "*");
  const target = agents.length ? agents.join(" / ") : "目标 Agent";
  const suggestedPrompt = `请使用“${skill.name}”Skill 完成：[填写任务与期望产出]。先说明适用范围、所需工具和验收方式，再开始执行。`;
  const how = node("article");
  how.append(node("h5", { text: "如何调用" }));
  if (skill.invocation) {
    how.append(
      node("p", { text: "Skill 元数据声明了以下调用入口：" }),
      node("code", { className: "local-invocation", text: skill.invocation }),
    );
  } else {
    how.append(node("p", {
      text: `未声明命令语法。在 ${target} 的任务中明确点名该 Skill；不要据名称臆测 /slash-command。`,
    }));
  }
  const prompt = node("div", { className: "local-usage-prompt" });
  prompt.append(node("code", { text: suggestedPrompt }));
  const copy = node("button", { className: "button button-quiet", text: "复制调用提示", attributes: { type: "button" } });
  copy.addEventListener("click", () => copyText(suggestedPrompt, "调用提示已复制"));
  prompt.append(copy);
  how.append(prompt);
  grid.append(when, how);
  section.append(
    grid,
    node("p", { className: "local-usage-boundary", text: "适用时机来自 Skill 的声明字段；未声明时只提供保守提示。具体步骤、权限与副作用必须以原文和目标 Agent 的规则为准。" }),
  );
  return section;
}

function renderLocalDocumentEvidence(payload, skill) {
  const wrapper = node("div", { className: "document-evidence" });
  const status = node("div", { className: "document-status-row" });
  status.append(
    node("span", { className: "document-severity", text: "本机只读 · 未执行" }),
    node("span", { text: `sha256:${payload.contentHash.slice(0, 16)}…` }),
    node("span", { text: payload.truncated ? "已按阅读上限截断" : "已读取完整文档" }),
  );
  wrapper.append(status);
  const reader = node("div", { className: "document-source-reader local-document-source" });
  reader.append(node("header", {}, [
    node("span", { text: `${skill.name}/SKILL.md` }),
    node("small", { text: "显式读取 · 不执行指令" }),
  ]));
  const lineList = node("ol", { className: "document-source-lines", attributes: { "aria-label": `${skill.name} 本机原文` } });
  ...
  for (const [index, value] of lines.slice(0, visibleLimit).entries()) {
    lineList.append(node("li", {
      dataset: { line: String(index + 1) },
      attributes: { value: String(index + 1) },
    }, [node("code", { text: value || " " })]));
  }
  reader.append(lineList);
  if (lines.length > visibleLimit) {
    reader.append(node("p", { className: "document-reader-limit", text: `阅读器仅展开前 ${visibleLimit.toLocaleString("zh-CN")} 行。` }));
  }
  wrapper.append(
    reader,
    node("p", { className: "document-review-warning", text: "原文可能包含不可信指令、脚本或外部引用；这里仅供人工审阅，不代表安全或可运行。" }),
  );
  return wrapper;
}

function renderLocalDocumentReview(entry) {
  const { skill } = entry;
  const key = localSkillDocumentKey(entry);
  const review = state.catalog.localDocument;
  const payload = review.cache[key];
  const loading = review.loadingKey === key;
  const error = review.errorKey === key ? review.error : "";
  const section = node("section", { className: "specimen-section ecosystem-document-review local-document-review" });
  const heading = node("header", { className: "document-review-heading" });
  const copy = node("div");
  copy.append(
    node("p", { className: "eyebrow", text: "本机使用依据" }),
    node("h4", { text: "按需读取 Skill 原文" }),
    node("p", { text: "只有点击后才读取这一份已扫描文档；内容限量返回浏览器，不会执行其中指令。" }),
  );
  const button = node("button", {
    className: "button button-primary",
    text: loading ? "正在读取原文……" : payload ? "重新读取原文" : "读取原文",
    attributes: { type: "button", ...(loading ? { disabled: "" } : {}) },
  });
  button.addEventListener("click", () => loadLocalSkillDocument(entry, { refresh: Boolean(payload) }));
  heading.append(copy, button);
  section.append(heading);
  if (loading) {
    section.append(node("div", { className: "document-review-loading", attributes: { "aria-live": "polite" } }, [
      node("i", { attributes: { "aria-hidden": "true" } }),
      node("span", { text: "正在读取单份本机 SKILL.md……" }),
    ]));
  } else if (error) {
    section.append(node("div", { className: "document-review-error" }, [
      node("strong", { text: "本机原文暂时不可用" }),
      node("p", { text: error }),
    ]));
  } else if (payload) {
    section.append(renderLocalDocumentEvidence(payload, skill));
  } else {
    section.append(node("div", { className: "document-review-empty" }, [
      node("strong", { text: "尚未读取原文" }),
      node("p", { text: "需要判断具体步骤、工具权限或副作用时，再显式读取并人工核对。" }),
    ]));
  }
  return section;
}
...
function renderLocalCatalogDetail(entry) {
  ...
  heading.append(
    node("p", {
      className: "eyebrow",
      text: `${(skill.providers || [skill.provider]).join(" / ")} · ${skill.scope} · ${skill.sourceKind} · ${ROOT_STABILITY[skill.rootStability] || skill.rootStability}`,
    }),
  ...
  facts.append(
    ...
    specimenFact("发现于 Agent", (skill.providers || [skill.provider]).join(", ")),
  ...
  elements.catalogDetail.append(renderLocalUsageGuide(skill));
  ...
  elements.catalogDetail.append(renderLocalDocumentReview(entry));
...
function applyCatalogMode() {
  ...
  elements.catalogDescription.textContent = ecosystem
    ...
    : "查看本机每份 Skill 的作用、适用时机、调用提示、来源、副本和冲突；原文仅在显式选择后读取。";
...
function reviewedEcosystemDocument(itemId, skillName) {
  return state.catalog.ecosystem.document.cache[ecosystemDocumentKey(itemId, skillName)] || null;
}
...
async function recordEcosystemCandidate(item, skillName, gap, rationale) {
  if (!state.activeWorkflow) throw new Error("请先选择工作流");
  const reviewed = reviewedEcosystemDocument(item.id, skillName);
  if (!reviewed?.document?.sha256) throw new Error("请先读取并审阅这份 Skill 原文");
  const workflow = await (await api(`/api/workflows/${encodeURIComponent(state.activeWorkflow.id)}/external-candidates`, {
    ...
    body: JSON.stringify({
      ...
      rationale,
      reviewedContentHash: reviewed.document.sha256,
    }),
  })).json();
  applyWorkflow(workflow);
  await fetchWorkflowList();
...
async function recordEcosystemChainCandidates(item, skillNames, gap, rationale) {
  if (!state.activeWorkflow) throw new Error("请先选择工作流");
  const reviewedContentHashes = Object.fromEntries(skillNames.map((skillName) => [
    skillName,
    reviewedEcosystemDocument(item.id, skillName)?.document?.sha256 || "",
  ]));
  if (Object.values(reviewedContentHashes).some((hash) => !hash)) {
    throw new Error("请先逐项读取并审阅组合链原文");
  }
  const workflow = await (await api(`/api/workflows/${encodeURIComponent(state.activeWorkflow.id)}/external-candidates`, {
    method: "POST",
    body: JSON.stringify({
...
function renderEcosystemChainCandidateForm(item) {
  ...
  const updateChainState = () => {
    ...
    for (const member of memberRows) {
      ...
      const recordable = item.recordableSkills.includes(member.skillName);
      const reviewed = Boolean(reviewedEcosystemDocument(item.id, member.skillName)?.document?.sha256);
      ...
      if (local.found) {
        ...
        status = "disabled";
        label = reviewed ? "本机已禁用 · 原文已审阅" : "本机已禁用 · 待审阅";
        if (recordable) {
          pendingSkills.push(member.skillName);
          if (!reviewed) pendingReviewSkills.push(member.skillName);
        }
      } else if (recordable) {
        status = reviewed ? "available" : "review-required";
        label = reviewed ? "原文已审阅 · 可记录" : "待原文审阅";
        pendingSkills.push(member.skillName);
        if (!reviewed) pendingReviewSkills.push(member.skillName);
      }
    ...
    submit.disabled = !gap || !pendingSkills.length || pendingReviewSkills.length > 0;
    if (pendingReviewSkills.length) submit.textContent = `先审阅 ${pendingReviewSkills.length} 个成员原文`;
    else if (pendingSkills.length) submit.textContent = `记录 ${pendingSkills.length} 个未覆盖成员`;
...
function renderEcosystemCandidateForm(item) {
  ...
  const updateSubmit = () => {
    ...
    const reviewed = Boolean(reviewedEcosystemDocument(item.id, skillSelect.value)?.document?.sha256);
    submit.disabled = Boolean(duplicate) || !reviewed;
    submit.textContent = duplicate ? "该缺口已记录" : reviewed ? "记录为缺口候选" : "先读取并审阅原文";
  };
...
function riskLabel(risk) {
  return ({
    ...
    "reviewed-content-missing": "缺少已审阅内容指纹，禁止安装",
    "reviewed-content-high-risk": "已审阅原文存在高风险静态线索，禁止安装",
  })[risk] || risk;
}
...
function renderInstallationItem(item, editable) {
  ...
  if (item.reviewedContentHash) {
    facts.append(node("div", {}, [
      node("span", { text: "已审阅指纹" }),
      node("code", { text: `sha256:${item.reviewedContentHash}` }),
    ]));
  }
  card.append(facts);
...
function renderInstallationReceipt() {
  ...
  if (plan.reassessment?.length) {
    ...
    for (const item of plan.reassessment) {
      reassessment.append(node("span", { text: `${item.targetAgent} · 匹配 ${Math.round(item.matchScore * 100)}% · 文本证据 ${Math.round((item.evidencedCoverageRatio ?? item.coverageRatio) * 100)}% · 人工确认 ${Math.round((item.confirmedCoverageRatio || 0) * 100)}% · 缺失 ${item.missingRequiredCapabilities}` }));
    }
...
async function quarantineInstallationItem(itemId) {
  const plan = latestInstallationPlan();
  if (!window.confirm("断开 SkillMesh 创建的 Agent 链接，并将托管来源移入隔离区？原始本地 Skill 不会删除。")) return;
...
function renderNoWorkflow() {
  ...
  elements.stages.replaceChildren(node("div", {
    ...
    text: "让已连接的 Agent 通过 MCP 提交工作流草案。草案出现后，你可以在这里核对能力证据并生成执行方案。",
  }));
...
async function generatePlan({ preserveSelection = false } = {}) {
  ...
  try {
    ...
    renderWorkflowState();
    persistWorkspace({ silent: true });
    if (elements.quickDeckDialog.open) {
      await loadQuickDeckContext();
    }
    if (elements.catalogDialog.open) renderCatalog();
  } finally {
...
playbookUi = createPlaybookUi({
  ...
  onWorkflowChange: (workflow) => applyWorkflow(workflow, { syncPlaybook: false }),
  toast,
});
...
elements.quickDeckButton.addEventListener("click", handle(openQuickDeck));
elements.quickUseBack.addEventListener("click", closeQuickUsePanel);
elements.quickUseForm.addEventListener("submit", handle(async (event) => {
  event.preventDefault();
  await submitQuickUse();
}));
for (const control of [elements.quickUseTask, elements.quickUseOutput, elements.quickUseAgent]) {
  control.addEventListener(control === elements.quickUseAgent ? "change" : "input", updateQuickUsePreview);
}
elements.quickDeckPlaybook.addEventListener("click", () => {
  elements.quickDeckDialog.close();
  elements.quickDeckPlaybook.blur();
  document.querySelector("#playbook-button")?.click();
});
elements.quickDeckCatalog.addEventListener("click", () => {
  elements.quickDeckDialog.close();
  openCatalogDialog();
});
elements.quickDeckDialog.addEventListener("close", () => {
  state.quickDeck.selectedItem = null;
  elements.quickDeckLayout.dataset.view = "cards";
  elements.quickUsePanel.hidden = true;
});
...
async function boot() {
  ...
  try {
    ...
    await scan();
    const workflows = await fetchWorkflowList();
    await migrateLegacyQuickSkillPreferences();
