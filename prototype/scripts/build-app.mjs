...
const outputPath = path.join(root, "dist", "skillmesh-workbench.html");

async function renderBundle() {
  const [template, css, bundle] = await Promise.all([
    fs.readFile(path.join(root, "app", "workbench.html"), "utf8"),
    fs.readFile(path.join(root, "app", "workbench.css"), "utf8"),
    build({
      entryPoints: [path.join(root, "app", "workbench.js")],
      bundle: true,
      write: false,
      minify: true,
      format: "iife",
...
if (check) {
  ...
  if (existing !== rendered) {
    console.error("SkillMesh MCP App bundle is stale. Run npm run build:app.");
    process.exitCode = 1;
  } else {
    const digest = crypto.createHash("sha256").update(rendered).digest("hex").slice(0, 12);
    console.log(`SkillMesh MCP App bundle is current (${digest}).`);
  }
} else {
